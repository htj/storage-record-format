...
A record should only represent a single identity. This identity either be a
...
consumed by the listed user in the context of the group information.
...
record, e.g., ''disk'' or tape. This allows for explicit accounting for
different types for backend storage. This field should not be used of the data
accounted for in the record is on different media types.
...
If the property is included in the record, the record should account for all
usage in the directory and only that directory.
...
A duration indicating for how long time the measurement is valid. The record
must only be used accounting for the time frame in which it is valid. The
generation of records should be performed in such a way, that gaps should not
occur. Note that a new record can be ``nullified'' if a newer record is
manifested, i.e., the most recent information should be used.
...
\item The SpaceAllocated field type MUST be a positive integer.
\end{itemize}
...
\begin{itemize}
\item The SpaceReserved field type MUST be a positive integer.
\end{itemize}
